  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (key === 'login') {
        ...
        setTimeout(onClose, 1000); // Đóng modal sau 1 giây
      } else {
        ...
        setTimeout(() => {
          setKey('login'); // Chuyển sang tab login
          onClose(); // Đóng modal
        }, 1000);
      }
    } catch (err) {
          ...
          <Tab eventKey="register" title="Đăng ký">
            <Form onSubmit={handleSubmit}>
              <Form.Group className="mb-3" controlId="registerName">
                ...
                <Form.Control type="text" name="registerName" placeholder="Nhập họ và tên" required />
              </Form.Group>
              <Form.Group className="mb-3" controlId="registerEmail">
                <Form.Label>Email</Form.Label>
                <Form.Control type="email" name="registerEmail" placeholder="Nhập email" required />
              </Form.Group>
              <Form.Group className="mb-3" controlId="registerPassword">
                <Form.Label>Mật khẩu</Form.Label>
                <Form.Control type="password" name="registerPassword" placeholder="Nhập mật khẩu" required />
              </Form.Group>
              <Form.Group className="mb-3" controlId="registerConfirmPassword">
                <Form.Label>Xác nhận mật khẩu</Form.Label>
                <Form.Control type="password" name="registerConfirmPassword" placeholder="Xác nhận mật khẩu" required />
              </Form.Group>
              <Button variant="primary" type="submit" className="w-100">
                Đăng ký
              </Button>
