    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (homestayId) {
          ...
          const ratingResponse = await axios.get(`http://localhost:8080/api/reviews/overall-rating/${homestayId}`);
          ratings[homestayId] = ratingResponse.data || 0;
          ...
          newOffers = await Promise.all(rooms.map(async (room) => {
            ...
            return {
              ...
              overallRating: ratings[homestayId],
            };
          }));
        } else {
          const response = await axios.get(
          ...
          if (validCoordinates.length > 0) {
            ...
            if (distanceFromCenter > 5) {
              setMapCenter(getFallbackCoordinates());
            } else {
          ...
          newOffers = await Promise.all(response.data.map(async (hs) => {
            ...
            const ratingResponse = await axios.get(`http://localhost:8080/api/reviews/overall-rating/${hs.id}`);
            ratings[hs.id] = ratingResponse.data || 0;
                          ...
                          <Card.Body className="d-flex flex-column justify-content-between">
                            <div>
                              ...
                              <p className="text-muted small mb-2">
                                Tổng đánh giá Homestay: {item.overallRating ? item.overallRating.toFixed(1) : 'Chưa có'}★
                              </p>
